...
refined_pred_data = []
for index in range(len(adata_sedr.obs['SEDR_leiden'].tolist())):
    refined_pred_data.append([index, int(adata_sedr.obs['SEDR_leiden'].tolist()[index])])
np.savetxt(os.path.join(f'{params.save_path}', f'pred_types.txt'), np.array(refined_pred_data), fmt='%3d', delimiter='\t')
